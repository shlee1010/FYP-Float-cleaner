...
if "col_backups" not in st.session_state:
    st.session_state["col_backups"] = {}

if "last_applied_col" not in st.session_state:
    st.session_state["last_applied_col"] = None
...
with st.sidebar:
    ...
    if st.session_state["raw_data"] is not None:
        ...
        if numeric_cols:
            ...
            default_index = 0
            if st.session_state["last_applied_col"]:
                last = st.session_state["last_applied_col"]
                last_display = f"✅ {last}" if last in st.session_state["change_log"] else last
                if last_display in display_options:
                    default_index = display_options.index(last_display)
                st.session_state["last_applied_col"] = None
            selected_option = st.selectbox("Select Target Column", display_options, index=default_index)
...
if st.session_state["raw_data"] is None:
    ...
    **4. Check Impact & Save**
    ...
    with tab_stat:
        ...
        st.dataframe(stat_df.style.format({"Original": "{:.4f}", "Cleaned": "{:.4f}", "Delta": "{:.4f}"}).map(highlight_delta, subset=['Delta']).background_gradient(subset=['Delta'], cmap='RdBu_r', vmin=-1, vmax=1), use_container_width=True, height=350)
    with tab_corr:
    ...
    with col_action:
        ...
        if is_cleaned:
            ...
            if st.button("↩️ Rollback (Undo)", type="secondary", use_container_width=True):
                backup = st.session_state.get("col_backups", {}).get(target_col)
                if backup:
                    # Restore full state including any deleted rows
                    st.session_state["raw_data_full"] = backup["full"].copy()
                    st.session_state["raw_data"] = backup["sample"].copy()
                    del st.session_state["col_backups"][target_col]
                elif st.session_state["original_data_full"] is not None:
                    # Fallback: restore from original load
                    st.session_state["raw_data_full"] = st.session_state["original_data_full"].copy()
                    st.session_state["raw_data"] = st.session_state["original_data"].copy()
                del st.session_state["change_log"][target_col]
                st.session_state["last_applied_col"] = target_col
            ...
            if st.button("🚀 Apply to Full Dataset", type="primary", use_container_width=True):
                with st.spinner("Processing full dataset... Please wait."):
                    # Backup current state before applying (for proper rollback including deleted rows)
                    st.session_state["col_backups"][target_col] = {
                        "full": st.session_state["raw_data_full"].copy(),
                        "sample": st.session_state["raw_data"].copy()
                    }
